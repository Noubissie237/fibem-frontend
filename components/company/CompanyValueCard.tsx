...
import { CompanyValue, getCompanyValueContent } from "@/lib/company";
...
const iconMap: Record<string, React.ReactNode> = {
  ...
  puzzle: <IconPuzzle className="w-6 h-6" />,
  shield: <IconShield className="w-6 h-6" />,
  star: <IconStar className="w-6 h-6" />,
  users: <IconUsers className="w-6 h-6" />,
};

const colorMap: Record<string, string> = {
  sparkles: "bg-purple-100 text-purple-600",
  puzzle: "bg-brand-blue-100 text-brand-blue",
  shield: "bg-emerald-100 text-emerald-600",
  star: "bg-amber-100 text-amber-600",
  users: "bg-rose-100 text-rose-600",
};

export function CompanyValueCard({ value, lang }: CompanyValueCardProps) {
  const content = getCompanyValueContent(value.id, lang);
  if (!content) return null;

  return (
    <Card hover padding="lg" className="text-center">
      <div className={`w-14 h-14 rounded-xl ${colorMap[value.icon]} flex items-center justify-center mx-auto mb-4`}>
        {iconMap[value.icon]}
      </div>
